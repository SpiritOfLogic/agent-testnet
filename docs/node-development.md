...
**Using existing applications instead of custom Go code?** If you're wrapping an off-the-shelf application (Gitea, DokuWiki, etc.) rather than writing a custom binary, use `testnet-toolkit certs fetch` to write certificates to disk, then point nginx or Caddy at them. See the [Node Toolkit](node-toolkit-design.md) design document and the [Toolkit Reference](toolkit-reference.md) for details.
...
**Wrapping an existing application?** For confining an off-the-shelf application's outbound traffic to the testnet without writing custom Go code, use `testnet-toolkit sandbox run`. It creates a network namespace where only testnet VIPs are reachable. Use `testnet-toolkit seed urls` to discover what domains exist. See the [Node Toolkit](node-toolkit-design.md) and the [sandboxed crawler deployment guide](guide-deploy-crawler.md).
